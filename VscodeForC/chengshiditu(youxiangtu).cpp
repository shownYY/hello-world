...
#include <stdlib.h>
#include <iostream>
#define INF 999999  //宏定义
using namespace std;
//针对有向图
int book[101], e[101][101];
int n;
int resmin = 9999999;//全局变量
int path[100]={0}; /// 用来保存路径  全局变量全部的值都是0
void dfs(int cur, int dis)//深度优先搜索
{
    int i;
...
int main()
{
    ...
    book[1] = 1; //标记1号城市已经在路径中  //标记数组
    path[1] = 1; // 用来保存路径
    dfs(1, 0); //1表示当前所在城市标号，0表示当前已经走过的路程
    cout << "最短路径的长度是:" << resmin << endl;
    return 0;
}
/*
5 8
